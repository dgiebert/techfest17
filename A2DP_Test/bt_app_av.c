...
{
  if (++m_pkt_cnt % 100 == 0) {
        log_e("audio data pkt cnt %u", m_pkt_cnt);
    }
}
...
void bt_app_rc_ct_cb(esp_avrc_ct_cb_event_t event, esp_avrc_ct_cb_param_t *param)
{
    ...
    default:
        log_e("avrc invalid cb event: %d", event);
        break;
    }
}
...
static void bt_av_hdl_a2d_evt(uint16_t event, void *p_param)
{
    log_d("%s evt %d", __func__, event);
    esp_a2d_cb_param_t *a2d = NULL;
    switch (event) {
    case ESP_A2D_CONNECTION_STATE_EVT: {
        a2d = (esp_a2d_cb_param_t *)(p_param);
        log_i("a2dp conn_state_cb, state %d", a2d->conn_stat.state);
        break;
    }
    case ESP_A2D_AUDIO_STATE_EVT: {
        a2d = (esp_a2d_cb_param_t *)(p_param);
        log_i("a2dp audio_state_cb state %d", a2d->audio_stat.state);
    ...
    case ESP_A2D_AUDIO_CFG_EVT: {
        a2d = (esp_a2d_cb_param_t *)(p_param);
        log_i("a2dp audio_cfg_cb , codec type %d", a2d->audio_cfg.mcc.type);
        // for now only SBC stream is supported
        if (a2d->audio_cfg.mcc.type == ESP_A2D_MCT_SBC) {
            log_i("audio player configured");
        }
        break;
    }
    default:
        log_e("%s unhandled evt %d", __func__, event);
        break;
    }
}
...
static void bt_av_hdl_avrc_evt(uint16_t event, void *p_param)
{
    ...
    case ESP_AVRC_CT_CONNECTION_STATE_EVT: {
        ...
        log_i("avrc conn_state evt: state %d, feature 0x%x, [%02x:%02x:%02x:%02x:%02x:%02x]",
